...
func (lbp *LeaderBoardProcessor) Process(round uint64, price map[string]float64) {
	lbp.lock.Lock()
	defer lbp.lock.Unlock()
	db := db.GetConnection()
...
func calculateLeaderBoard(account model.Account, positions []model.Position, price map[string]float64) model.LeaderBoard {
	...
	totalPnL += (totalAmount - config.INITIAL_USD_BALANCE)
	leaderBoard := model.LeaderBoard{
		Round:   account.Round,
		Address: account.Address,
		PnL:     totalPnL,
		RoI:     totalPnL / config.INITIAL_USD_BALANCE,
	}
	return leaderBoard
}
